...
function CalculDistance({ point1, point2, mettreAJourDistance }) {
  const calculerDistance = (lat1, lon1, lat2, lon2) => {
    ...
    const phy1 = (lat1 * Math.PI) / 180; // latitude du premier point convertie en radians
    const phy2 = (lat2 * Math.PI) / 180; // de même pour le deuxième point

    const deltaPhy = ((lat2 - lat1) * Math.PI) / 180; // différence entre les latitudes (en degrés) convertie en radians

    const deltaLambda = ((lon2 - lon1) * Math.PI) / 180; // de même pour les longitudes

    // Calcul de la formule de Haversine pour la distance entre deux points sur la surface de la Terre
    const a =
      Math.sin(deltaPhy / 2) * Math.sin(deltaPhy / 2) + // calcul de la composante sinus de la différence de latitude
      Math.cos(phy1) * Math.cos(phy2) * Math.sin(deltaLambda / 2) * Math.sin(deltaLambda / 2); // calcul de la composante sinus de la différence de longitude
